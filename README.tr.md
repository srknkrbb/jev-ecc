...
# ya da elle:
claude plugin marketplace add srknkrbb/jev-ecc && claude plugin install jev-ecc@jev-ecc
# yerel gelistirme: JEV_ECC_LOCAL=~/work/jev-ecc sh install.sh
```
Guncelleme: `.claude-plugin/{plugin,marketplace}.json` icinde `version` artir → `claude plugin marketplace update jev-ecc && claude plugin update jev-ecc@jev-ecc` (ayni surum yeniden kopyalanmaz).
